...
class LoftQConfig:
    ...
    # add
    bits_pattern: Dict[str, int] = field(default_factory=dict, metadata={"help": "Layer-specific quantization bits"})
    def to_dict(self):
        """Returns a dictionary representation of this config."""
        return {"loftq_bits": self.loftq_bits, "loftq_iter": self.loftq_iter, "bits_pattern": self.bits_pattern}
...
class LoraConfig(PeftConfig):
    ...
    # )
    ## add
    loftq_config: Union[LoftQConfig, dict] = field(default_factory=dict, metadata={"help": "Configuration of LoftQ with layer-specific bits settings"})
    ...
    #         if self.init_lora_weights == "loftq" and not self.loftq_config.bits_pattern:
    #             raise ValueError("bits_pattern must be provided in loftq_config when `init_lora_weights` is set to 'loftq'.")
    ...
    def __post_init__(self):
        ...
        if self.use_dora and self.megatron_config:
            raise ValueError("DoRA does not support Megatron core configurations. Please set `use_dora=False`.")
        ...
        if self.init_lora_weights == "loftq":
            ...
            if not isinstance(self.loftq_config, LoftQConfig):
                raise ValueError("`loftq_config` must be an instance of LoftQConfig when `init_lora_weights` is 'loftq'.")
            if not self.loftq_config.bits_pattern:
                raise ValueError("`bits_pattern` must be provided in `loftq_config` when `init_lora_weights` is set to 'loftq'.")
